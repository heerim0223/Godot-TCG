extends CanvasLayer
# Self-contained settings window. A persistent "Settings" button in the
# corner opens a dimmed overlay panel; every control in it just forwards to
# GameSettings, which applies the change immediately and saves it to disk.
# Closing and reopening (or restarting the game) always shows the current
# GameSettings values, so this node holds no settings state of its own.
